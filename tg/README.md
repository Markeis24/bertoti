...
```mermaid
flowchart TD
    A["Front-end — HTML / CSS / JavaScript"] --> B["Controller — RestController"]
    B --> C["Service — regras de negocio e validacoes"]
    C --> D["Repository — Spring Data JPA"]
    D --> E["Entities — MySQL (IPEM_Control)"]

    style A fill:#2C0A4D,color:#fff,stroke:#6A0DAD,stroke-width:2px
    style B fill:#4B0E8C,color:#fff,stroke:#6A0DAD,stroke-width:2px
    style C fill:#6A0DAD,color:#fff,stroke:#6A0DAD,stroke-width:2px
    style D fill:#8A2BE2,color:#fff,stroke:#6A0DAD,stroke-width:2px
    style E fill:#A45EE5,color:#fff,stroke:#6A0DAD,stroke-width:2px
```
...
```mermaid
timeline
    title Minhas entregas como desenvolvedora backend
    section Sprint 1 - 20/03 a 04/04
        Cadastro de viaturas : validacao e excecoes
        Endpoint de tecnicos : consulta GET
        Motorista para Tecnico : reestruturacao de dominio
        Uso de veiculo : validacoes e listagem de ativos
    section Sprint 2 - 19/04 a 28/04
        Query agregada : GROUP BY por viatura
        DashboardService : Object para DTO
        Endpoint Chart.js : labels usos horas
        Historico de uso : entidade repository endpoint
    section Sprint 3 - 28/04
